...
import requests
import os
import tempfile # Pour une gestion propre des fichiers

# --- CONFIGURATION ---
st.set_page_config(page_title="Audio Perception AI - Pro", layout="wide")

# Récupération sécurisée des secrets (Configurable dans .streamlit/secrets.toml)
...
# Cache Streamlit pour éviter de recalculer si on change l'affichage
@st.cache_data(show_spinner=False)
def analyze_human_perception(file_path, original_filename):
    """Analyse avancée imitant la perception humaine."""
    y, sr = librosa.load(file_path, sr=22050)
    # Pré-accentuation pour simuler la sensibilité de l'oreille aux hautes fréquences
    ...
    # Profils de Krumhansl-Kessler pour la détection de tonalité
...
if uploaded_file:
    # Utilisation d'un fichier temporaire pour librosa
    ...
    with st.spinner(f"Analyse de : {uploaded_file.name}..."):
        try:
            # L'argument original_filename assure que le cache réagit aux nouveaux fichiers
            ...
            # Graphique Radar pour visualiser l'énergie par note
