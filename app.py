...
def analyze_audio_optimized(file_buffer):
    """Analyse optimisée avec la précision harmonique de l'original."""
    # Chargement direct depuis le buffer (plus rapide)
    y, sr = librosa.load(file_buffer, sr=22050)
    
    # Filtre de pré-emphase (simule l'audition humaine)
    y = librosa.effects.preemphasis(y)
    
    # Extraction CQT avec bins_per_octave=24 (Identique à ton original pour la précision)
    ...
    for i in range(12):
        ...
        if s_maj > best_score:
            best_score, key, tone = s_maj, notes[i], "Major"
        if s_min > best_score:
            best_score, key, tone = s_min, notes[i], "Minor"

    # Top 3 des notes dominantes pour le rapport
...
st.markdown("Analyse multi-fichiers haute précision (Profils harmoniques originaux).")
...
if uploaded_files:
        ...
        with st.expander(f"📊 Résultats pour : {f.name}", expanded=True):
            try:
                ...
                try:
                    ...
                    img_bytes = None
                    st.warning("Info : Installez 'kaleido' pour inclure le graphique dans Telegram.")
    ...
    st.info("Prêt pour l'analyse. Déposez un ou plusieurs fichiers audio.")
