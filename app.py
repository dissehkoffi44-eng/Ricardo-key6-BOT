...
def analyze_human_perception(file_path, original_filename):
    ...
    # Application d'un filtre de pré-emphase pour simuler l'oreille humaine (boost des hautes fréquences)
    ...
    # Corrélation croisée pour trouver la meilleure tonalité
    for i in range(12):
...
if uploaded_file:
    # Création d'un fichier temporaire pour que Librosa puisse le lire
    ...
    with st.spinner(f"Analyse de : {uploaded_file.name}..."):
        try:
            ...
            # Graphique Radar de l'énergie des notes
